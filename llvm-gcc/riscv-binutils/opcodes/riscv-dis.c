  ...
  for (; *d != '\0'; d++)
	...
	case 'C': /* RVC */
	  switch (*++d)
	    ...
	    case 't': /* RS2 x8-x15.  */
	    case 'x': /* RS2 x8-x15.  */
		// print (info->stream, "noway?"); //used in ADDI	      
		print (info->stream, "%s",
		     riscv_gpr_names[EXTRACT_OPERAND (CRS2S, l) + 8]);
	      break;
	//donghyeon C instructions are compressed ones; not needed
	    case 'r':
		print (info->stream, "no news is good news");	      
		print (info->stream, "%s",
	...
	case 't':
	  // print (info->stream, "test?"); // confirmed message printed
	  print (info->stream, "%s",
		 riscv_gpr_names[EXTRACT_OPERAND (RS2, l)]);
	  break;
	//donghyeon
	case 'r':
	  //print (info->stream, "why?"); // doesn't reach here for some reason. 
	...
	case 'R':
	  //print(info->stream, "one new is good news"); //donghyeon
